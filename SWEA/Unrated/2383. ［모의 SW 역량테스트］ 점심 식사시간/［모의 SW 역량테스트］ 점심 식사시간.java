	...
	public static void init() throws IOException{
		...
		for(int[] personLocation : personLocationList) {
			for(int stairIdx=0; stairIdx<2; stairIdx++) {
				int time = getManhattanDistance(stairLocationList[stairIdx], personLocation);
				timeToStairs[stairIdx][personCnt] = time;
			}
			personCnt++;
		}
		stairQueue = new LinkedList[2];
		minTime=Integer.MAX_VALUE;
	}
	
	public static void playStairGame() {
		stairQueue[0] = new LinkedList<>();
		stairQueue[1] = new LinkedList<>();
		...
		int maxTime = 0;
		for(int stairIdx=0; stairIdx<2; stairIdx++) {
			Collections.sort(stairQueue[stairIdx]);
			movingQueue = new ArrayDeque<>();
			int lastTime = 0;
			
			// 먼저 가능한 만큼 추가
			// 대기 1분 추가
			for(int personIdx=0; personIdx<3 && !stairQueue[stairIdx].isEmpty(); personIdx++) {
				int escapeTime = stairQueue[stairIdx].remove(0) + stairTime[stairIdx] + 1;
				movingQueue.add(escapeTime);
			}
			
			while(!stairQueue[stairIdx].isEmpty()) {
				int curTime = movingQueue.poll();
				int nxtTime = stairQueue[stairIdx].remove(0);
